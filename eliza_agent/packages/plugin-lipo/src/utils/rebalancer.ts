...
export interface RebalanceParams {
    ...
    nft_id?: bigint;
    userAddress?: string;
    confidenceMultiplier?: number; // Default 1.96 for 95% confidence
}
...
export interface RebalanceResult {
    ...
    newPositionTx?: ethers.ContractTransactionResponse;
 ...
 */
function calculateDecPriceChange(realized_vol: number): number {
 ...
 */
async function getCurrentTick(poolContract: ethers.Contract): Promise<number> {
...
export async function rebalanceWithRuntime(
    runtime: any, // Your IAgentRuntime type
    addresses: ContractAddresses,
    params: RebalanceParams
): Promise<RebalanceResult> {
